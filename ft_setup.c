...
/*   Updated: 2021/01/24 09:31:50 by mlabrayj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	set_resolution(char **params)
{
	...
	{
		t_win.resl.width = ft_atoiforresul(params[1]);
		t_win.resl.height = ft_atoiforresul(params[2]);
		if (t_win.resl.width < 0 || t_win.resl.height < 0)
			throwerror("Resolution must be great than (1x1)");
		t_win.resl.width = (t_win.resl.width > 3200)
							? 3200 : t_win.resl.width;
		t_win.resl.height = (t_win.resl.height > 1800)
							? 1800 : t_win.resl.height;
